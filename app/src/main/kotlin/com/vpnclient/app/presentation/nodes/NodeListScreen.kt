                ...
                uiState.nodes.isEmpty() && !uiState.isLoading -> {
                    ...
                    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                        Column(horizontalAlignment = Alignment.CenterHorizontally) {
                            Icon(
                                    imageVector =
                                            Icons.Default.Refresh, // Using Refresh icon instead of
                                    // VpnKey
                    ...
                    LazyColumn(
                        ...
                        // Node items
                        items(items = uiState.nodes, key = { node -> node.id }) { node ->
                            NodeItem(
                                    node = node,
...
@Composable
private fun ConnectionStatusCard(connectionState: ConnectionState, onDisconnect: () -> Unit) {
    Card(
        ...
        Column(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
            Text(
                    text =
                            when (connectionState) {
                                is ConnectionState.Connected ->
                                        "Connected to ${connectionState.nodeId}"
                                is ConnectionState.Connecting ->
                                        "Connecting to ${connectionState.nodeId}..."
...
@Composable
private fun getLatencyColor(latencyMs: Int): Color {
    return when {
        latencyMs < 50 -> Color(0xFF4CAF50) // Green
        latencyMs < 100 -> Color(0xFFFF9800) // Orange
